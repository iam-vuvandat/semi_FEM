...
@dataclass
class Output:
    mu_r: Union[float, np.ndarray]
    dmu_r_dB: Union[float, np.ndarray]
...
def lookup_BH_curve(
    ...
    if invert:
        mu_old = mu_result.copy()
        mu_result = 1.0 / (mu_old + 1e-30)
        
        if return_du_dB:
            dmu_result = -1.0 * (mu_result ** 2) * dmu_result

    if return_du_dB:
        dmu_result = dmu_result * np.sign(B_array)
