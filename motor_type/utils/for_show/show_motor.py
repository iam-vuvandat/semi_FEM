...
def _add_cylindrical_axes_static(pl, length=100):
    ...
    pl.add_mesh(pv.Arrow(start=origin, direction=[0, 0, 1], **arrow_params), color='#2980B9', name='axis_z', lighting=False)
    pl.add_mesh(pv.Arrow(start=origin, direction=[1, 0, 0], **arrow_params), color='#C0392B', name='axis_r', lighting=False)
    ...
    offset = length * 0.1
    pos_O = origin - np.array([offset*0.3, offset*0.3, 0])
    lbl_pos = [pos_O, [0, 0, length+offset], [length+offset, 0, 0], [p_end[0]+offset*0.5, p_end[1]+offset*0.5, 0]]
    pl.add_point_labels(lbl_pos, ["O", "z", "r", "θ"], font_size=20, always_visible=True, name='axis_labels')

# --- HÀM XỬ LÝ CHỈ SỐ LƯỚI ---
...
def show_motor(motor):
    ...
    # --- 2. PLOTTER ---
    pl = BackgroundPlotter(title="Motor Analysis Integration", window_size=(1600, 900))
    pl.set_background("#FFFFFF")
    _add_cylindrical_axes_static(pl, np.max(mesh_obj.r_nodes) * 1.2)
    ...
    class ViewerState:
        ...
        def init_static_layers(self):
            ...
            if geometry_obj and geometry_obj.geometry:
                for idx, seg in enumerate(geometry_obj.geometry):
                    ...
                    color = "#3498DB"
                    if "iron" in mat or "steel" in mat: color = "#D3D3D3"
        ...
        def _redraw_wireframe(self):
            pl.remove_actor("static_mesh_wire")
            pl.add_mesh(self.active_grid, style='wireframe', color='black', opacity=0.1, 
                        line_width=1, name="static_mesh_wire")
            if "static_mesh_wire" in pl.actors:
                pl.actors["static_mesh_wire"].SetVisibility(self.show_mesh_lines)

        def update_static_visibility(self):
            # Geometry
            if geometry_obj:
                for idx in range(len(geometry_obj.geometry)):
                    if f"geo_{idx}" in pl.actors: pl.actors[f"geo_{idx}"].SetVisibility(self.show_geometry)
            # Mesh
            if "static_mesh_wire" in pl.actors: pl.actors["static_mesh_wire"].SetVisibility(self.show_mesh_lines)
            # Axes
            for n in ['axis_z', 'axis_r', 'axis_arc', 'axis_labels']:
                if n in pl.actors: pl.actors[n].SetVisibility(self.show_axes)
            pl.render()
        ...
        # --- RENDER NETWORK (ĐÃ FIX LỖI NUMPY) ---
        ...
        def get_current_data(self):
            ...
            for idx, el in enumerate(el_lite):
                ...
                # --- [FIXED] SỬ DỤNG "is not None" CHO NUMPY ARRAY ---
                if "iron" in m: mat_sec[idx] = 1
                elif "magnet" in m: 
                    # Kiểm tra an toàn: magnetization_direction có tồn tại không
                    z_val = 0
                    if el.magnetization_direction is not None:
                        z_val = el.magnetization_direction[-1]
                    mat_sec[idx] = 2 if z_val >= 0 else 4
                elif "coil" in m: mat_sec[idx] = 3
                
                if el.flux_density_average is not None: 
                    b_sec[idx] = el.flux_density_average[-1]
    ...
    act_mesh = QAction("Mesh", pl.app_window); act_mesh.setCheckable(True); act_mesh.setChecked(False)
    ...
    act_bmap = QAction("B-Map", pl.app_window); act_bmap.setCheckable(True); act_bmap.setChecked(False)
    ...
    def add_slice(label, attr_show, attr_pos, check=False):
        ...
        limit_attr = f"max_{attr_pos.split('_')[1]}"
        dec = QAction("-", pl.app_window)
        dec.triggered.connect(lambda: (setattr(state, attr_pos, np.clip(getattr(state, attr_pos)-1, 0, getattr(state, limit_attr)-1)), state.render()))
        tb.addAction(dec)
        inc = QAction("+", pl.app_window)
        inc.triggered.connect(lambda: (setattr(state, attr_pos, np.clip(getattr(state, attr_pos)+1, 0, getattr(state, limit_attr)-1)), state.render()))
        tb.addAction(inc)
