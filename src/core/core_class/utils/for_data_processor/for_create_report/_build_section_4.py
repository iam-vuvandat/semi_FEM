...
def _build_section_4(story, record, config):
    """
    Generates Section 4: Performance Metrics Validation Summary.
    Extracts the pre-computed average mechanical power data and its relative error 
    from the motor record object and renders a structured monochrome table.
    """
    # 1. Thêm tiêu đề chương và mô tả ngắn sử dụng kiểu chữ Times New Roman hệ thống
    ...
    # 2. Trích xuất dữ liệu Công suất và Sai số tương đối trung bình từ thực thể record
    ...
    pow_err = f"{record.mechanical_power_average_error:.2f}%" if hasattr(record, "mechanical_power_average_error") else "N/A"
    ...
    kpi_rows = [
        ...
        [Paragraph("Average Mechanical Output Power", config.table_text), Paragraph(mbgrn_pow, config.table_text), Paragraph(fem_pow, config.table_text), Paragraph(pow_err, config.table_text)]
    ]
