...
def _build_axial_force(story, motor, config):
    story.append(Paragraph("5.8 Axial Force Transient Waveforms (On-Load Component)", config.h2_style))
    
    from ...for_update_report.update_axial_force_data import update_axial_force_data
    from .._fig_to_image_flowable import _fig_to_image_flowable
    
    fig_wave = motor.data_processor.plot_axial_force(plot=False)
    update_axial_force_data(data_processor=motor.data_processor)
    
    metrics = motor.record.axial_force_metrics if hasattr(motor.record, "axial_force_metrics") else None
    ...
    h_mrn = motor.record.axial_force_harmonic if hasattr(motor.record, "axial_force_harmonic") else None
    h_fem = motor.record.axial_force_harmonic_fem if hasattr(motor.record, "axial_force_harmonic_fem") else None
