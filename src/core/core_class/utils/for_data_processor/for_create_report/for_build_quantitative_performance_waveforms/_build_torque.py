...
def _create_metrics_table(metrics_data, unit, config):
    rows = [
        ...
        [Paragraph("Torque Ripple Ratio (%)", config.table_text), Paragraph(metrics_data['mbgrn_ripple'], config.table_text), Paragraph(metrics_data['fem_ripple'], config.table_text), Paragraph(metrics_data['error_ripple'], config.table_text)]
    ]
...
def _create_harmonic_table(h_mrn, h_fem, config, target_orders=[1, 3, 5, 7]):
    rows = [
        [Paragraph("<b>Harmonic Order</b>", config.table_header),
         Paragraph("<b>MBGRN Amp (Nm)</b>", config.table_header),
         Paragraph("<b>FEM Amp (Nm)</b>", config.table_header),
...
def _build_torque(story, motor, config):
    story.append(Paragraph("5.1 Electromagnetic Torque Transient Performance Profiles", config.h2_style))
    
    from ...for_update_report.update_torque_data import update_torque_data
    from .._fig_to_image_flowable import _fig_to_image_flowable
    
    fig_wave = motor.data_processor.plot_torque(plot=False)
    update_torque_data(data_processor=motor.data_processor)
    
    metrics = motor.record.torque_metrics if hasattr(motor.record, "torque_metrics") else None
    
    if metrics is not None:
        story.append(Paragraph("<b>Electromagnetic Torque Quantitative Verification Metrics</b>", config.body_style))
        story.append(_create_metrics_table(metrics, "Nm", config))
        story.append(Spacer(1, 14))
    
    h_mrn = motor.record.torque_harmonic if hasattr(motor.record, "torque_harmonic") else None
    h_fem = motor.record.torque_harmonic_fem if hasattr(motor.record, "torque_harmonic_fem") else None
    
    story.append(Paragraph("<b>Electromagnetic Torque Spectrum Harmonics Verification</b>", config.body_style))
