...
def _build_airgap_flux(story, motor, config):
    story.append(Paragraph("5.2 Airgap Flux Density Spatial Distribution (On-Load Component)", config.h2_style))
    
    metrics = motor.record.airgap_flux_density_metrics
    ...
    h_mrn = motor.record.airgap_flux_density_harmonic if hasattr(motor.record, "airgap_flux_density_harmonic") else None
    h_fem = motor.record.airgap_flux_density_harmonic_fem if hasattr(motor.record, "airgap_flux_density_harmonic_fem") else None
    ...
    fig_wave, fig_harm = motor.data_processor.plot_airgap_flux_density(plot=False)
