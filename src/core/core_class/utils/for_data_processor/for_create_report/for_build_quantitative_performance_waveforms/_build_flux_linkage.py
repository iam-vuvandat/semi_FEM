...
def _build_flux_linkage(story, motor, config):
    story.append(Paragraph("5.6 Core Winding Flux Linkage Transient Waveforms (On-Load Component)", config.h2_style))
    
    from ...for_update_report.update_flux_linkage_data import update_flux_linkage_data
    from .._fig_to_image_flowable import _fig_to_image_flowable
    
    fig_wave, fig_harm = motor.data_processor.plot_flux_linkage(plot=False)
    update_flux_linkage_data(data_processor=motor.data_processor)
    
    metrics = motor.record.flux_linkage_metrics if hasattr(motor.record, "flux_linkage_metrics") else None
    ...
    h_mrn = motor.record.flux_linkage_harmonic if hasattr(motor.record, "flux_linkage_harmonic") else None
    h_fem = motor.record.flux_linkage_harmonic_fem if hasattr(motor.record, "flux_linkage_harmonic_fem") else None
