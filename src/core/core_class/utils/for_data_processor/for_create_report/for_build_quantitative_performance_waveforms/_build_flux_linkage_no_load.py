...
def _create_harmonic_table(h_mrn, h_fem, config, target_orders=[1, 3, 5, 7]):
    rows = [
        [Paragraph("<b>Harmonic Order</b>", config.table_header),
         Paragraph("<b>MBGRN Amp (Wb)</b>", config.table_header),
         Paragraph("<b>FEM Amp (Wb)</b>", config.table_header),
...
def _build_flux_linkage_no_load(story, motor, config):
    story.append(Paragraph("5.5 Core Winding Flux Linkage Transient Waveforms (Open-Circuit No-Load Component)", config.h2_style))
    
    from ...for_update_report.update_flux_linkage_no_load_data import update_flux_linkage_no_load_data
    from .._fig_to_image_flowable import _fig_to_image_flowable
    
    fig_wave, fig_harm = motor.data_processor.plot_flux_linkage_no_load(plot=False)
    update_flux_linkage_no_load_data(data_processor=motor.data_processor)
    
    metrics = motor.record.flux_linkage_no_load_metrics if hasattr(motor.record, "flux_linkage_no_load_metrics") else None
    
    if metrics is not None:
        story.append(Paragraph("<b>Phase-A Winding Flux Linkage Verification Metrics</b>", config.body_style))
        story.append(_create_metrics_table(metrics, "Wb", config))
        story.append(Spacer(1, 14))
    
    h_mrn = motor.record.flux_linkage_no_load_harmonic if hasattr(motor.record, "flux_linkage_no_load_harmonic") else None
    h_fem = motor.record.flux_linkage_no_load_harmonic_fem if hasattr(motor.record, "flux_linkage_no_load_harmonic_fem") else None
    
    story.append(Paragraph("<b>Phase-A Flux Linkage Spatial Harmonics Verification</b>", config.body_style))
