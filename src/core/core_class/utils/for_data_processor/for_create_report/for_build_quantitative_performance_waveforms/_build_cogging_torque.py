...
def _build_cogging_torque(story, motor, config):
    story.append(Paragraph("5.2 Open-Circuit No-Load Cogging Torque Waveforms Profile", config.h2_style))
    
    from ...for_update_report.update_cogging_torque_data import update_cogging_torque_data
    from .._fig_to_image_flowable import _fig_to_image_flowable
    
    fig_wave = motor.data_processor.plot_cogging_torque(plot=False)
    update_cogging_torque_data(data_processor=motor.data_processor)
    
    metrics = motor.record.cogging_metrics if hasattr(motor.record, "cogging_metrics") else None
    
    if metrics is not None:
        story.append(Paragraph("<b>Cogging Torque Quantitative Verification Metrics</b>", config.body_style))
        story.append(_create_metrics_table(metrics, "Nm", config))
        story.append(Spacer(1, 14))
    
    h_mrn = motor.record.cogging_harmonic if hasattr(motor.record, "cogging_harmonic") else None
    h_fem = motor.record.cogging_harmonic_fem if hasattr(motor.record, "cogging_harmonic_fem") else None
    
    story.append(Paragraph("<b>Cogging Torque Spectrum Harmonics Verification</b>", config.body_style))
