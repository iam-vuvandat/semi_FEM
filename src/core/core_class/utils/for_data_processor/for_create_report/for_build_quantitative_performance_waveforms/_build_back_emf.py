...
def _build_back_emf(story, motor, config):
    story.append(Paragraph("5.4 Phase Winding Back Electromotive Force Transient Waveforms (On-Load Component)", config.h2_style))
    
    from ...for_update_report.update_back_emf_data import update_back_emf_data
    from .._fig_to_image_flowable import _fig_to_image_flowable
    
    fig_wave, fig_harm = motor.data_processor.plot_back_emf(plot=False)
    update_back_emf_data(data_processor=motor.data_processor)
    
    metrics = motor.record.back_emf_metrics if hasattr(motor.record, "back_emf_metrics") else None
    ...
    h_mrn = motor.record.back_emf_harmonic if hasattr(motor.record, "back_emf_harmonic") else None
    h_fem = motor.record.back_emf_harmonic_fem if hasattr(motor.record, "back_emf_harmonic_fem") else None
