...
def plot_axial_force(data_processor, horizontal_axis = "mechanical_position", 
    ...
    has_sf = hasattr(record, "axial_force")
    ...
    if has_fem:
        ...
        ax.plot(x_fem, val_fem, color='gray', linestyle='--', alpha=0.6, label='FEM Axial Force')
        ax.axhline(y=np.mean(val_fem), color='gray', linestyle=':', alpha=0.5)

    if has_sf:
        data_sf = record.axial_force
        x_sf, x_label = get_x_axis(data_sf[-1, :])
        val_sf = data_sf[0, :]
        ax.plot(x_sf, val_sf, color='#AA3377', label='SF Axial Force', linewidth=2.5)
        ax.axhline(y=np.mean(val_sf), color='black', linestyle='--')

    ax.set_xlabel(x_label, fontsize=s.label_size)
    ax.set_ylabel(r'Axial Force ($N$)', fontsize=s.label_size)
