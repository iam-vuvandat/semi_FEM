...
def update_back_emf_data(data_processor):
    motor = data_processor.motor
    record = motor.record

    e_mbgrn = record.back_emf if hasattr(record, "back_emf") else None
    e_fem = record.back_emf_fem if hasattr(record, "back_emf_fem") else None

    ea_mbgrn = e_mbgrn[0, :] if e_mbgrn is not None else None
    ea_fem = e_fem[0, :] if e_fem is not None else None

    record.back_emf_metrics = _compute_component_metrics(ea_mbgrn, ea_fem)
    return True