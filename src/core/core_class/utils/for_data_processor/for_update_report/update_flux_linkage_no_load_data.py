...
def update_flux_linkage_no_load_data(data_processor):
    motor = data_processor.motor
    record = motor.record
    max_h = 15

    psi_mbgrn = record.flux_linkage_no_load if hasattr(record, "flux_linkage_no_load") else None
    psi_fem = record.flux_linkage_no_load_fem if hasattr(record, "flux_linkage_no_load_fem") else None

    psia_mbgrn = psi_mbgrn[2, :] if psi_mbgrn is not None else None
    psia_fem = psi_fem[2, :] if psi_fem is not None else None

    record.flux_linkage_no_load_metrics = _compute_component_metrics(psia_mbgrn, psia_fem)
