...
def update_airgap_flux_density_data(data_processor):
    """
    Executes post-processing calculations on 3D airgap flux density vector components
    (Radial, Tangential, Axial) under on-load condition and updates motor.record.
    """
    motor = data_processor.motor
    record = motor.record

    b_mbgrn = record.airgap_flux_density if hasattr(record, "airgap_flux_density") else None
    b_fem = record.airgap_flux_density_fem if hasattr(record, "airgap_flux_density_fem") else None
    ...
    record.airgap_flux_density_metrics = metrics_data
    return True