...
def update_axial_force_no_load_data(data_processor):
    motor = data_processor.motor
    record = motor.record

    f_mbgrn = record.axial_force_no_load if hasattr(record, "axial_force_no_load") else None
    f_fem = record.axial_force_no_load_fem if hasattr(record, "axial_force_no_load_fem") else None

    val_mbgrn = f_mbgrn[0, :] if f_mbgrn is not None else None
    val_fem = (f_fem[0, :] * -1) if f_fem is not None else None

    record.axial_force_no_load_metrics = _compute_component_metrics(val_mbgrn, val_fem)
    return True