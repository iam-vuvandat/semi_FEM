...
class Mechanical:
    def __init__(self, motor):
        self.motor = motor
        self.geometry_data = self.motor.geometry_data
        self.mechanical_data = self.motor.mechanical_data

        self.shaft_speed = self.motor.mechanical_data.shaft_speed
        self.current_position = 0.0
        self.cogging_period_mech = find_cogging_period(geometry_data = self.geometry_data).period_mech
        print(f"cogging period:{self.cogging_period_mech}")

        self.symmetry_factor = find_symmetry_factor(geometry_data= self.geometry_data).symmetry_factor
        print(f"symmetry factor: {self.symmetry_factor}")
