...
class ViewerState:
    ...
    def init_static_layers(self):
        if self.geometry_obj and self.geometry_obj.geometry:
            for idx, seg in enumerate(self.geometry_obj.geometry):
                ...
                self.pl.add_mesh(seg.mesh, color=color, opacity=op, lighting=True, 
    ...
    def _redraw_wireframe(self):
        ...
        self.pl.add_mesh(self.active_grid, style='wireframe', color='black', opacity=0.1, 
    ...
    def toggle_bmap_btn(self, state):
        if not self.has_results: return
    ...
    def render(self):
        ...
        try:
            ...
            render_mesh = pv.UnstructuredGrid()
    ...
    def update_text_info(self):
        ...
        frame_txt = self.current_frame if self.has_results else "N/A"
        info = f"Frame: {frame_txt}\nSym: {'ON' if self.use_symmetry else 'OFF'}\n" \
        ...
        self.pl.add_text(info, position="upper_left", font_size=9, name="info_text", color='black')
    ...
    def save_screenshot_hd(self):
        ...
        if f: self.pl.screenshot(f, transparent_background=False, scale=4); print(f"Saved to {f}")
