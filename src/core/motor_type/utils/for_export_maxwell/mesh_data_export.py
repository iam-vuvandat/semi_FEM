...
def mesh_data_export(motor, m3d):
    ...
    if not os.path.exists(mstat_path):
        print(f"\033[91mError: Mesh stats file not found at {mstat_path}\033[0m")
        return
    ...
    print(f"\033[92mSuccess: Mesh statistics recorded -> Total: {motor.record.mesh_data_fem.total_elements}, Min: {motor.record.mesh_data_fem.min_element_length} {motor.record.mesh_data_fem.unit}, Max: {motor.record.mesh_data_fem.max_element_length} {motor.record.mesh_data_fem.unit}\033[0m")