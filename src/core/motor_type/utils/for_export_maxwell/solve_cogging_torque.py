...
from src.core.motor_type.utils.for_export_maxwell.disable_excitation import disable_excitation
from src.core.motor_type.utils.for_export_maxwell.cogging_torque_export import cogging_torque_export

def solve_cogging_torque(m3d,motor,winding):
    if motor.calculation_data.general_options.solve_cogging:
        ...
        # Disable excitation
        disable_excitation(m3d = m3d, current= winding)


        shaft_speed = motor.mechanical_data.shaft_speed # rpm
        shaft_speed *= 2 * math.pi / 60 # rad/s

        theta_sweep = motor.mechanical.cogging_period_mech # rad 
        ...
        if motor.maxwell_export_option.solver_option.solve_immediately :
            for setup_name in m3d.setup_names:
                ...
                print("Cogging torque export starting")
