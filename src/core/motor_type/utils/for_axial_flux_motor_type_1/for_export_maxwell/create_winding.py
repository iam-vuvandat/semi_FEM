...
def create_winding_section(m3d, section_radius, slot_index, slot_arc, offset_x, z, section_name=None):
...
def create_winding(motor, m3d):
    ...
    turns = int(getattr(wdg_data, 'turns', 15))
    ...
    if sl_matrix is not None:
        ...
        for p_idx in range(phase):
            w_name = f"Winding_Ph{p_idx}"
            
            m3d.assign_winding(
                current=current_function[p_idx],
                is_solid=False,
                name=w_name
            )
            ...
            for s_idx in range(int(slot_num)):
                if sl_matrix[s_idx, p_idx] > 0:
                    ...
                    term_name = f"Terminal_{coil_name}"
                    exc_name = f"Exc_{coil_name}"
                    ...
                    section_name = create_winding_section(m3d, wire_rad, s_idx, slot_arc, r_in_ext, z_layers[0])
                    ...
                    create_winding_section(m3d, wire_rad, s_idx, slot_arc, r_in_ext, z_layers[0], section_name=term_name)
                    ...
                    m3d.add_winding_coils(assignment=w_name, coils=exc_name)