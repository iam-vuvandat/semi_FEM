...
def init_ui(input_tab=None):
    ...
    def update_summary_display():
        ...
        if db:
            summary = (
                ...
                f"<p><b>HARD MAGNETIC (Magnet):</b><br>{db.magnet.name}<br>"
                f"&nbsp;&nbsp;• Mur: {db.magnet.relative_permeance}<br>"
                f"&nbsp;&nbsp;• Hc: {db.magnet.coercivity} A/m</p>"
                f"<p><b>SOFT MAGNETIC (Iron):</b><br>{db.iron.name}<br>"
    ...
    def handle_refresh():
        if motor is None:
            return
        if motor.motor_state_manager.ready_state.material_database:
    ...
    btn_recreate.setObjectName("secondaryButton")
    btn_recreate.setFixedHeight(35)
