...
# 1. Tu thong lien ket (Full: d, q, abc)
axs[0].plot(theta_flux, flux[0, :], 'k--', label='$\Psi_d$ (Direct)', linewidth=1.5)
axs[0].plot(theta_flux, flux[1, :], 'k-', label='$\Psi_q$ (Quadrature)', linewidth=1.5)
for j in range(aft.winding_data.phase):
    axs[0].plot(theta_flux, flux[2 + j, :], color=colors[j % 3], label=f'Pha {chr(65+j)}', alpha=0.7)
axs[0].set_title("Từ thông liên kết (Flux Linkage: dq & abc)", fontweight='bold')
...
# 5. Dong dien (Full: d, q, abc)
axs[4].plot(theta_curr, currents[0, :], 'r--', label='$I_d$ (Direct)', linewidth=1.5)
axs[4].plot(theta_curr, currents[1, :], 'b-', label='$I_q$ (Quadrature)', linewidth=1.5)
for j in range(aft.winding_data.phase):
    axs[4].plot(theta_curr, currents[2 + j, :], color=colors[j % 3], label=f'Phase {chr(65+j)}', alpha=0.7)
axs[4].set_title("Dòng điện (Currents: dq & abc)", fontweight='bold')
axs[4].set_ylabel("Current (A)")
axs[4].set_xlabel("Vị trí Rotor (rad)")
axs[4].legend(loc='upper right', fontsize='small', ncol=2)
