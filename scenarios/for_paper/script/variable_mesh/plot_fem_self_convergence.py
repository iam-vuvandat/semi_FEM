...
def plot_fem_self_convergence(file_name_array, io, figures_dir=None):
    ...
    torque_true = record_ref.torque_fem if hasattr(record_ref, 'torque_fem') and record_ref.torque_fem is not None else None
    cogging_true = getattr(record_ref, 'cogging_fem', None) or getattr(record_ref, 'cogging_torque_fem', None)
    flux_true = record_ref.flux_linkage_fem if hasattr(record_ref, 'flux_linkage_fem') and record_ref.flux_linkage_fem is not None else None
    bgap_true = record_ref.airgap_flux_density_fem if hasattr(record_ref, 'airgap_flux_density_fem') and record_ref.airgap_flux_density_fem is not None else None
    ...
    for file_name in file_name_array:
        ...
        tets = record.mesh_data_fem.total_elements if hasattr(record, 'mesh_data_fem') and record.mesh_data_fem is not None else 0
        elements_count.append(tets)
        
        if torque_true is not None and hasattr(record, 'torque_fem') and record.torque_fem is not None:
            nrmse_val = get_waveform_nrmse(torque_true, record.torque_fem, num_points=100, row_index=0)
            torque_nrmse_list.append(nrmse_val)
        else:
            torque_nrmse_list.append(0.0)

        cogging_data = getattr(record, 'cogging_fem', None) or getattr(record, 'cogging_torque_fem', None)
        ...
        if flux_true is not None and hasattr(record, 'flux_linkage_fem') and record.flux_linkage_fem is not None:
            nrmse_val = get_waveform_nrmse(flux_true, record.flux_linkage_fem, num_points=100, row_index=2)
            flux_nrmse_list.append(nrmse_val)
        else:
            flux_nrmse_list.append(0.0)

        if bgap_true is not None and hasattr(record, 'airgap_flux_density_fem') and record.airgap_flux_density_fem is not None:
            nrmse_val = get_waveform_nrmse(bgap_true, record.airgap_flux_density_fem, num_points=100, row_index=2)
    ...
    ax.set_xlabel(r'Total FEM Tetrahedra ($\times 10^3$)', fontsize=s.label_size)
    ax.set_ylabel('NRMSE (%)', fontsize=s.label_size)
    plt.title('3D-FEM Multi-Parameter NRMSE Self-Convergence', fontsize=s.title_size)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, '3d_fem_self_convergence.png'), dpi=300, bbox_inches='tight')
    plt.show()